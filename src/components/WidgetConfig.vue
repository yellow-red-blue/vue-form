      ...
      <el-form-item :label="$t('fm.config.widget.option')" v-if="Object.keys(data.options).indexOf('options')>=0">
        ...
        <template v-if="data.options.remote">
          <div>
            ...
            <el-select size="mini" style="" v-model="data.options.remoteFunc">
              <el-option v-for="(op, index) in remoteFuncList" :label="op.label" :value="op.value" :key="index"></el-option>
            </el-select>
...
export default {
  ...
  data () {
    return {
      ...
      remoteFuncList: [{label: '测试1', value: 'functest1'}, {label: '测试2', value: 'functest2'}]
    }
  },
  computed: {
    show () {
