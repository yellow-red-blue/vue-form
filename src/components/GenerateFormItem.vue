    ...
    // if (this.widget.options.remote && this.remote[this.widget.options.remoteFunc]) {
      ...
      const getOptions = (id) => {
        return new Promise(resolve => {
          let data = []
          if(id === 'functest1') {
            data = [{label: '我是测试1', value: '1'}]
          } else if(id === 'functest2') {
            data = [{label: '我是测试2', value: '2'}]
          }
          resolve(data)
        })
        
      }
      getOptions(this.widget.options.remoteFunc).then(res => {
        this.widget.options.remoteOptions = res.map(item => {
          return {
